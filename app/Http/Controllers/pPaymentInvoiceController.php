...
class PaymentInvoiceController extends Controller
{
    ...
    public function dealerInvoiceIndex($id){
        $item = $this->stockOutItem->find($id);
        $stockItemTotalPrice = StockItem::whereDealerId($item->dealer_id)->orWhere('item_id', $item->item_id)->latest()->pluck('temp_total')->first();
        // dd($stockItemTotalPrice);
    ...
    public function storeDealerDues(Request $request){
        ...
        $preStock = StockItem::whereNull('retailer_id')->whereDealerId($request->dealer_id)->orderBy('created_at', 'desc')->first();
        DB::transaction(function () use ($request, $due, $preStock){
            if (!empty($due)){
                ...
                $due ->update($request->except('temp_total'));
            } elseif (!empty($preStock)){
                $duesInsert = new Request($request->except('temp_total'));
                $duesInsert->merge(['dealer_id' => $request->dealer_id, 'admin_id' => auth('admin')->user()->id, 'due' => $preStock->temp_total + $request->due]);
                // dd($due, $preStock, $request->all());
                Payment::create($duesInsert->all());
            } else {
                $dueInsert = new Request($request->except('temp_total'));
                $dueInsert->merge(['dealer_id' => $request->dealer_id, 'admin_id' => auth('admin')->user()->id]);
                // dd($due, $preStock, $request->all());
                Payment::create($request->except('temp_total'));
            }
