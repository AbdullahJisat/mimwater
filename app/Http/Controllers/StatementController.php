...
class StatementController extends Controller
{
    ...
    public function indexRetailer(Request $request)
    {
        ...
        $retailerStock = StockOutItem::whereRetailerId($request->retailer_id)->whereBetween('created_at', [$start, $end])->get();
    ...
    public function dailyRetailerStatement(){
       $dailyStatement = Statement::with('retailer')->whereDate('created_at', Carbon::today())->get();
        return view('backend.pages.statement.daily-retailer', compact('dailyStatement'));
    }

    public function dateRetailerStatement(Request $request)
    {
        $start = Carbon::createFromFormat('Y-m-d', $request->start)->startOfDay();
        $end = Carbon::createFromFormat('Y-m-d', $request->end)->endOfDay();
        $dailyStatement = Statement::with('retailer')->whereNotNull('retailer_id')->whereBetween('created_at', [$start, $end])->get();
        return view('backend.pages.statement.daily-retailer', compact('dailyStatement', 'start', 'end'));
